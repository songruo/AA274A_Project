...
class AStar(object):
    ...
    def is_free(self, x):
        ...
        """
        ########## Code starts here ##########
        if (x[0] < self.statespace_lo[0]) or (x[1] < self.statespace_lo[1]) or (x[0] > self.statespace_hi[0]) or (x[1] > self.statespace_hi[1]) or (not self.occupancy.is_free(x)):
            return False
        else:
            return True
        ########## Code ends here ##########
    ...
    def get_neighbors(self, x):
        ...
        neighbors = []
        ########## Code starts here ##########
        neighborNorth = self.snap_to_grid((x[0], x[1] + self.resolution))
        neighborSouth = self.snap_to_grid((x[0], x[1] - self.resolution))
        neighborEast = self.snap_to_grid((x[0] + self.resolution, x[1]))
        neighborWest = self.snap_to_grid((x[0] - self.resolution, x[1]))
        neighborNE = self.snap_to_grid((x[0] + self.resolution, x[1] + self.resolution))
        neighborNW = self.snap_to_grid((x[0] - self.resolution, x[1] + self.resolution))
        neighborSE = self.snap_to_grid((x[0] + self.resolution, x[1] - self.resolution))
        neighborSW = self.snap_to_grid((x[0] - self.resolution, x[1] - self.resolution))
        neighbor = {neighborNorth, neighborSouth, neighborEast, neighborWest, neighborNE, neighborNW, neighborSE, neighborSW}
        for i in neighbor:
            if self.is_free(i):
                neighbors.append(i)
        ########## Code ends here ##########
    ...
    def solve(self):
        ...
        while len(self.open_set) > 0:
            ...
            #print(xcurrent, self.x_goal)
            #print("goal:{}".format(self.x_goal))
            if np.abs(np.array(xcurrent) - np.array(self.x_goal)).max() < 1e-5:
